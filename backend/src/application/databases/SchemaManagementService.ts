...
function validateColumnName(name: string) {
  if (!name || typeof name !== 'string') {
    throw new DatabaseError('SQLITE_SCHEMA_INVALID', 'Column name is required.', false);
  }

  if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(name)) {
    throw new DatabaseError('SQLITE_SCHEMA_INVALID', 'Invalid column name.', false);
  }

  return name;
}

function validateColumnType(type: string) {
  const normalized = String(type || '').trim().toUpperCase();
  if (!normalized) {
    throw new DatabaseError('SQLITE_SCHEMA_INVALID', 'Column type is required.', false);
  }

  if (!/^[A-Z0-9_\s(),]+$/.test(normalized)) {
    throw new DatabaseError('SQLITE_SCHEMA_INVALID', 'Invalid column type.', false);
  }
...
function extractTableColumnName(segment: string) {
...
function replaceColumnType(segment: string, nextType: string) {
  const trimmed = segment.trim();
  const match = trimmed.match(/^(?:"([^"]+)"|`([^`]+)`|\[([^\]]+)\]|'([^']+)'|([A-Za-z_][A-Za-z0-9_]*))\s+(.*)$/s);
  if (!match) {
    throw new DatabaseError('SQLITE_SCHEMA_INVALID', `Unable to parse column definition: ${segment}`, false);
  }
...
function buildCreateTableSql(originalSql: string, tableName: string, tempTableName: string, defs: string[]) {
...
function isTableConstraintSegment(segment: string) {
...
export class SchemaManagementService {
  ...
  async deleteTable(databaseId: string, tableName: string, actorId?: string) {
    ...
    if (client instanceof SqliteClient) {
      ...
      try {
        await (client as LibsqlClient).execute(`DROP TABLE IF EXISTS ${quoteIdentifier(safeTableName)}`);
      } catch (error: any) {
        pool.evictOnError(database.id, error);
  ...
  async renameTable(databaseId: string, tableName: string, nextTableName: string, actorId?: string) {
    const database = await this.databaseRepo.findOneByOrFail({ id: databaseId });
    const safeTableName = validateTableName(tableName);
    const safeNextTableName = validateTableName(nextTableName);

    if (safeTableName === safeNextTableName) {
      throw new DatabaseError('SQLITE_SCHEMA_INVALID', 'The new table name must be different.', false);
    }
  ...
  async addColumn(databaseId: string, tableName: string, input: { name: string; type: string; notnull?: boolean; defaultValue?: string; unique?: boolean }, actorId?: string) {
    const database = await this.databaseRepo.findOneByOrFail({ id: databaseId });
    const safeTableName = validateTableName(tableName);
    ...
    if (input.unique) {
      throw new DatabaseError('SQLITE_SCHEMA_INVALID', 'SQLite does not allow UNIQUE on ADD COLUMN directly. Use a migration instead.', false);
    }
  ...
  async renameColumn(databaseId: string, tableName: string, columnName: string, nextColumnName: string, actorId?: string) {
    const database = await this.databaseRepo.findOneByOrFail({ id: databaseId });
    const safeTableName = validateTableName(tableName);
    ...
    if (safeColumnName === safeNextColumnName) {
      throw new DatabaseError('SQLITE_SCHEMA_INVALID', 'The new column name must be different.', false);
    }
  ...
  async deleteColumn(databaseId: string, tableName: string, columnName: string, actorId?: string) {
    const database = await this.databaseRepo.findOneByOrFail({ id: databaseId });
    const safeTableName = validateTableName(tableName);
    const safeColumnName = validateColumnName(columnName);
    const pool = ConnectionPool.getInstance();
    ...
    if (client instanceof SqliteClient) {
      ...
      try {
        await (client as LibsqlClient).execute(`ALTER TABLE ${quoteIdentifier(safeTableName)} DROP COLUMN ${quoteIdentifier(safeColumnName)}`);
      } catch (error: any) {
        pool.evictOnError(database.id, error);
        throw new DatabaseError('LIBSQL_ERROR', error.message || 'Failed to delete column', true);
      }
    }
  ...
  async changeColumnType(databaseId: string, tableName: string, columnName: string, nextType: string, actorId?: string) {
    const database = await this.databaseRepo.findOneByOrFail({ id: databaseId });
    const safeTableName = validateTableName(tableName);
    const safeColumnName = validateColumnName(columnName);
    ...
    try {
      await this.rebuildTableWithClient(
        client instanceof SqliteClient ? client : client as LibsqlClient,
        safeTableName,
        (parts) => this.transformColumnType(parts, safeColumnName, safeNextType),
      );
    } catch (error: any) {
      pool.evictOnError(database.id, error);
      if (client instanceof SqliteClient) {
  ...
  private async rebuildWithoutColumn(client: SqliteClient, tableName: string, columnName: string) {
  ...
  private async rebuildTableWithClient(
    ...
    const createSql = String((tableRows[0] as any)?.sql || '');
    if (!createSql) {
      throw new DatabaseError('SQLITE_SCHEMA_INVALID', 'Table definition not found.', false);
    }

    const triggerRows = await fetchRows(`SELECT name FROM sqlite_master WHERE type='trigger' AND tbl_name='${tableName}'`);
    if ((triggerRows || []).length > 0) {
      throw new DatabaseError('SQLITE_SCHEMA_UNSUPPORTED', 'This table has triggers. Schema reconstruction is blocked for safety.', false);
    }
    ...
    const next = transform({
      ...
      indexes: indexRows as Array<{ name: string; sql: string }>,
    });
  ...
  private transformWithoutColumn(parts: { createSql: string; columnDefs: string[]; tableConstraints: string[]; indexes: Array<{ name: string; sql: string }> }, columnName: string) {
    ...
    if (nextColumnDefs.length === parts.columnDefs.length) {
      throw new DatabaseError('SQLITE_SCHEMA_INVALID', `Column "${columnName}" not found.`, false);
    }
    ...
    if (affectedConstraint) {
      throw new DatabaseError('SQLITE_SCHEMA_UNSUPPORTED', `Column "${columnName}" is referenced by a table constraint and cannot be dropped safely.`, false);
    }
  ...
  private transformColumnType(parts: { createSql: string; columnDefs: string[]; tableConstraints: string[]; indexes: Array<{ name: string; sql: string }> }, columnName: string, nextType: string) {
    let updated = false;
    const nextColumnDefs = parts.columnDefs.map((segment) => {
      const currentName = extractTableColumnName(segment);
      if (currentName !== columnName) return segment;
      updated = true;
  ...
  private parseCreateTableSql(createSql: string) {
    const firstParen = createSql.indexOf('(');
    const lastParen = createSql.lastIndexOf(')');
    if (firstParen < 0 || lastParen < firstParen) {
      throw new DatabaseError('SQLITE_SCHEMA_INVALID', 'Unable to parse table definition.', false);
    }
